...
#include "ana/TValidationModuleJJ.hh"

ClassImp(TValidationModuleJJ)
//-----------------------------------------------------------------------------
TValidationModuleJJ::TValidationModuleJJ(const char* name, const char* title):
...
//-----------------------------------------------------------------------------
TValidationModuleJJ::~TValidationModuleJJ() {
}


//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookTimeClusterHistograms   (TimeClusterHist_t*   Hist, const char* Folder){
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookHelixHistograms   (HelixHist_t*   Hist, const char* Folder){
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookTrackSeedHistograms   (TrackSeedHist_t*   Hist, const char* Folder){
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookClusterHistograms(ClusterHist_t* Hist, const char* Folder) {
//   char name [200];
//   char title[200];
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookGenpHistograms(GenpHist_t* Hist, const char* Folder) {
//   char name [200];
//   char title[200];
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookTrackHistograms(TrackHist_t* Hist, const char* Folder) {
//   char name [200];
//   char title[200];
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookEventHistograms(EventHist_t* Hist, const char* Folder) {
  //  char name [200];
  //  char title[200];
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::BookSimpHistograms(SimpHist_t* Hist, const char* Folder) {
  //  char name [200];
  //  char title[200];
...
void TValidationModuleJJ::BookHistograms() {
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::FillEventHistograms(EventHist_t* Hist) {
...
//--------------------------------------------------------------------------------
void TValidationModuleJJ::FillTrackSeedHistograms(TrackSeedHist_t*   Hist, TStnTrackSeed*    TrkSeed){
...
//--------------------------------------------------------------------------------
void TValidationModuleJJ::FillTimeClusterHistograms(TimeClusterHist_t*   Hist, TStnTimeCluster*    TimeCluster){
...
// function to fill Helix block
//--------------------------------------------------------------------------------
void TValidationModuleJJ::FillHelixHistograms(HelixHist_t*   Hist, TStnHelix*    Helix){
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::FillClusterHistograms(ClusterHist_t* Hist, TStnCluster* Cluster) {
  int   row, col;
  float  x, y, z, r;
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::FillGenpHistograms(GenpHist_t* Hist, TGenParticle* Genp) {
  int    gen_id;
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::FillSimpHistograms(SimpHist_t* Hist, TSimParticle* Simp) {
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::FillTrackHistograms(TrackHist_t* Hist, TStnTrack* Track) {
  ...
  //  printf("TValidationModuleJJ::FillTrackHistograms: track charge is not defined yet\n");
  Hist->fQ->Fill(-1);
...
//-----------------------------------------------------------------------------
int TValidationModuleJJ::BeginJob() {
//-----------------------------------------------------------------------------
...
//_____________________________________________________________________________
int TValidationModuleJJ::BeginRun() {
...
//_____________________________________________________________________________
void TValidationModuleJJ::FillHistograms() {
...
//-----------------------------------------------------------------------------
int TValidationModuleJJ::Event(int ientry) {
...
//-----------------------------------------------------------------------------
    ...
    if ((tp->fEp > 0) && (track->fEp > 0) && (fabs(tp->fEp-track->fEp) > 1.e-6)) {
      GetHeaderBlock()->Print(Form(" TValidationModuleJJ ERROR: tp->fEp = %10.5f  track->fEp = %10.5f",tp->fEp,track->fEp));
    }
//-----------------------------------------------------------------------------
// PID likelihoods
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void TValidationModuleJJ::Debug() {
...
//_____________________________________________________________________________
int TValidationModuleJJ::EndJob() {
  int ntrk = fTrackBlock->NTracks();
  printf("----- NTracks = %i\n",ntrk);
  printf("----- end job: ---- %s\n",GetName());
  return 0;
...
//_____________________________________________________________________________
void TValidationModuleJJ::Test001() {
